...
function scoringBlock(review: DeathmatchReport): string {
  const s = review.score!;
    return [
      `*상대 평가 점수*`,
      ``,
      `챔피언: 기준점 100점 (CPPV: ${won(s.championCPPV)} | 저장공유율: ${formatPercent(s.championSaveRate)} | CTR: ${formatPercent(s.championCTR)})`,
      `챌린저: 총점 *${s.challengerHS.toFixed(1)}점* (CPPV: ${won(s.challengerCPPV)} | 저장공유율: ${formatPercent(s.challengerSaveRate)} | CTR: ${formatPercent(s.challengerCTR)})`,
...
function buildAttrFormula(s: DeathmatchScore): string {
  ...
  if (srC === 0 && srCh > 0) {
    return `② AttrScore: 챔피언 저장+공유 0, 챌린저 있음 → 20점`;
  }
...
function buildCTRFormula(s: DeathmatchScore): string {
  ...
  if (ctrC === 0 && ctrCh === 0) {
    return `③ CTRScore: 양쪽 클릭 0 → 20점 (중립)`;
  }
  if (ctrC === 0 && ctrCh > 0) {
    return `③ CTRScore: 챔피언 클릭 0 → 40점 (최대)`;
  }
  if (ctrC > 0 && ctrCh === 0) {
    return `③ CTRScore: 챌린저 클릭 0 → 0점 (최소)`;
  }
