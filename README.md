...
| **SaveShareRate** (저장공유율) | `(saves + shares) ÷ reach` | 광고를 본 사람 중 저장+공유한 비율. 높을수록 좋음 |
| **Frequency** (광고 빈도) | `impressions ÷ reach` | 같은 사람이 평균 몇 번 광고를 봤는지. 2.0 이상이면 피로도 발생 |

> CPPV 예시: 10,000원으로 방문 100명 → CPPV = 100원/방문
> SaveShareRate 예시: 도달 5,000명 중 저장 50회 + 공유 30회 → (50+30)/5000 = 1.6%
...
| **SaveShareRate** | `(saves + shares) ÷ reach` | Proportion of reached users who saved or shared the post. Higher is better |
...
> SaveShareRate example: 5,000 reach, 50 saves + 30 shares → (50+30)/5000 = 1.6%
